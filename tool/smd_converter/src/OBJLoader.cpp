...
// Desc : 文字列からディレクトリを削除
//-----------------------------------------------------------------------
const char* CutDirectoryPath( const char* filename )
{
    std::string temp(filename);
...
bool OBJMESH::LoadOBJFile(const char *filename)
{
    ...
    uint32_t dwFaceIndex = 0;
    uint32_t dwFaceCount = 0;
    uint32_t dwCurSubset = 0;
    ...
    for( ;; )
    {
        ...
        else if ( 0 == strcmp( buf, "f" ) )
        {
            ...
            uint32_t p[4] = {UINT32_MAX}, t[4] = {UINT32_MAX}, n[4] = {UINT32_MAX};
            OBJVERTEX vertex;
            dwFaceIndex++;
            dwFaceCount++;
            int count = 0;
            ...
            {
                count++;    //　頂点数を数える
                memset( &vertex, 0, sizeof( OBJVERTEX ) );
    ...
    //　インデックスデータをコピー
    m_Indices = new uint32_t [ m_NumIndices ];
...
bool OBJMESH::LoadMTLFile( const char* filename )
{
    ...
    for( ;; )
    {
        ...
        else if ( 0 == strcmp( buf, "map_Ka" ) )
        {
            ...
            if (m_TextureList.find(mapKaName) != std::end(m_TextureList))
            { m_TextureList[mapKaName] = texture_id++; }
        }
        // Diffuse Map
        else if ( 0 == strcmp( buf, "map_Kd" ) )
        {
            ...
            if (m_TextureList.find(mapKdName) != std::end(m_TextureList))
            { m_TextureList[mapKdName] = texture_id++; }
        }
        // Specular Map
        else if ( 0 == strcmp( buf, "map_Ks" ) )
        {
            ...
            if (m_TextureList.find(mapKsName) != std::end(m_TextureList))
            { m_TextureList[mapKsName] = texture_id++; }
        }
        // Bump Map
               ...
               || ( 0 == strcmp( buf, "bump"     ) ) )
        {
            ...
            if (m_TextureList.find(mapBumpName) != std::end(m_TextureList))
            { m_TextureList[mapBumpName] = texture_id++; }
        ...
        else if ( 0 == strcmp( buf, "disp" ) )
        {
            ...
            if (m_TextureList.find(displacementMapName) != std::end(m_TextureList))
            { m_TextureList[displacementMapName] = texture_id++; }
        }
...
void OBJMESH::WriteDirect( FILE* pFile )
{
    ...
    memcpy( &header.Magic, SMD_FILE_TAG, sizeof( uint8_t ) * 4 );
    header.Version        = SMD_CURRENT_VERSION;
    header.VertexCount    = m_NumVertices;
    header.MaterialCount  = m_NumMaterials;
    header.TextureCount   = uint32_t(m_TextureList.size());
    header.TriangleCount  = m_NumIndices / 3;
    ...
    for (size_t i = 0; i<m_NumSubsets; ++i)
    {
        ...
        {
            // 頂点インデックスを算出.
            uint32_t idx = m_Subsets[i].faceStart + j;
            ...
            for (size_t k = 0; k < 3; ++k)
            {
                ...
                // 位置座標を設定.
                vtx.Position.x = m_Vertices[idx + k].position.x;
                vtx.Position.y = m_Vertices[idx + k].position.y;
                vtx.Position.z = m_Vertices[idx + k].position.z;

                // 法線ベクトルを設定.
                vtx.Normal.x = m_Vertices[idx + k].normal.x;
                vtx.Normal.y = m_Vertices[idx + k].normal.y;
                vtx.Normal.z = m_Vertices[idx + k].normal.z;

                // テクスチャ座標を設定.
                vtx.Texcoord.x = m_Vertices[idx + k].texcoord.x;
                vtx.Texcoord.y = m_Vertices[idx + k].texcoord.y;
    ...
    {
        fwrite(&vertices[i], sizeof(vertices[i]), 1, pFile);
    }

    // テクスチャファイル名を書き込む.
    auto itr = m_TextureList.begin();
    while(itr != m_TextureList.end())
    {
        SMD_TEXTURE texture;
        strcpy_s(texture.Path, itr->first.c_str());
    ...
    {
        SMD_MATERIAL mat;
        mat.Type        = SMD_MATERIAL_TYPE(m_Materials[i].materialType);
        mat.Color.x     = m_Materials[i].diffuse.x;
        mat.Color.y     = m_Materials[i].diffuse.y;
        mat.Color.z     = m_Materials[i].diffuse.z;
        mat.Emissive.x  = m_Materials[i].emissive.x;
        mat.Emissive.y  = m_Materials[i].emissive.y;
        mat.Emissive.z  = m_Materials[i].emissive.z;
        mat.Shininess   = m_Materials[i].shininess;
        mat.Ior         = m_Materials[i].ior;

        uint32_t textureId = SMD_INVALID_ID;
        if (m_TextureList.find(m_Materials[i].diffuseMapName) != std::end(m_TextureList))
        { textureId = m_TextureList[m_Materials[i].diffuseMapName]; }

        mat.ColorMap   = textureId;
